...
* **Strict Guardrails Mapping:** Interlaces explicit system instructions ensuring flavor harmony, converting bartending metrics (`oz`, `dashes`) for spirits vs. ultra-precise metrics (`ml`, `grams`) for additives, and enforcing pristine text syntax over clinical medical jargon.
...
### 3. The Archives (`/modules/journal`)
...
│   ├── dashboard.js             # Main navigation hub rendering endpoint
...
└── server.js                    # Core application lifecycle & pipeline orchestrator
...
Open your browser and enter `http://localhost:3000` to breach the entrance.